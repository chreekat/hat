...
main = do
    ...
    case msrc of
        Nothing -> do
            ...
            exitFailure
        -- The runner resolves the dev build via `cabal list-bin hat`. We do NOT
        -- resolve hat from PATH here: on a dev box PATH holds the system-installed
        -- hat (the user's real server), which must never be driven by tests.
        Just _ ->

...
- **Emulator ForeignPtr finalizer** (`Emulator.hsc:340`) — `FC.newForeignPtr`
  frees the libvterm object, the callbacks struct, and every wrapper `FunPtr`
...
| 6 | C | `Emulator.hsc:340` foreign allocs | libvterm obj, callbacks struct, wrapper FunPtrs | ForeignPtr finalizer | none | leave as-is. Model finalizer pattern. |

...
spec = do
    ...
    -- Ported from libvterm's own regression suite (t/*.test) to keep hat's
    -- emulator coverage tracking upstream. Each mirrors one behavior a
    -- .test script pins, observed through hat's grid/cursor snapshot.
    describe "upstream libvterm regressions" $ do
        -- t/21state_tabstops.test: HT advances to the next multiple-of-8 column.
        it "advances the cursor to the next tab stop on HT" $ do
            e <- new80x24
            _ <- feedStr e "\tX"
            scr <- snapshot e
            (screenCell scr Pos { row = 0, col = 8 }).text `shouldBe` "X"
            scr.cursor `shouldBe` Pos { row = 0, col = 9 }

        -- t/21state_tabstops.test: successive tabs step 8 columns each.
        it "steps eight columns per tab across successive tabs" $ do
            e <- new80x24
            _ <- feedStr e "\tX\tY"
            scr <- snapshot e
            (screenCell scr Pos { row = 0, col = 8 }).text `shouldBe` "X"
            (screenCell scr Pos { row = 0, col = 16 }).text `shouldBe` "Y"

        -- t/13state_edit.test (!ICH): CSI N @ opens N blank cells at the cursor,
        -- shifting the rest of the line right.
        it "inserts blank cells and shifts the line right (ICH)" $ do
            e <- new80x24
            _ <- feedStr e "ABC\ESC[3G\ESC[2@"
            scr <- snapshot e
            rowText scr 0 `shouldBe` "AB  C"

        -- t/13state_edit.test (!DCH): CSI N P deletes N cells at the cursor,
        -- pulling the tail of the line left.
        it "deletes cells and pulls the line left (DCH)" $ do
            e <- new80x24
            _ <- feedStr e "ABCDE\ESC[2G\ESC[2P"
            scr <- snapshot e
            rowText scr 0 `shouldBe` "ADE"

        -- t/13state_edit.test (!ECH): CSI N X blanks N cells in place without
        -- moving the cursor or the surrounding text.
        it "erases cells in place without shifting (ECH)" $ do
            e <- new80x24
            _ <- feedStr e "ABCDE\ESC[2G\ESC[2X"
            scr <- snapshot e
            rowText scr 0 `shouldBe` "A  DE"
            scr.cursor `shouldBe` Pos { row = 0, col = 1 }

        -- t/13state_edit.test (!IL): CSI L opens a blank line at the cursor row,
        -- scrolling the lines below it down.
        it "opens a blank line and scrolls lower rows down (IL)" $ do
            e <- new80x24
            _ <- feedStr e "A\r\nB\ESC[1;1H\ESC[L"
            scr <- snapshot e
            rowText scr 0 `shouldBe` ""
            rowText scr 1 `shouldBe` "A"
            rowText scr 2 `shouldBe` "B"

        -- t/22state_save.test: DECSC (ESC 7) then DECRC (ESC 8) restore the
        -- saved cursor position.
        it "saves and restores the cursor with DECSC/DECRC" $ do
            e <- new80x24
            _ <- feedStr e "\ESC[2;2H\ESC7\ESC[5;5H\ESC8"
            scr <- snapshot e
            scr.cursor `shouldBe` Pos { row = 1, col = 1 }

        -- t/12state_scroll.test: a linefeed at the bottom of a DECSTBM scroll
        -- region scrolls only that region, leaving rows outside it untouched.
        it "scrolls only within a DECSTBM region on linefeed" $ do
            e <- new80x24
            _ <- feedStr e "\ESC[4;1Houtside\ESC[1;3r\ESC[1;1Htop\r\nmid\r\nbot\r\nX"
            scr <- snapshot e
            rowText scr 0 `shouldBe` "mid"
            rowText scr 1 `shouldBe` "bot"
            rowText scr 2 `shouldBe` "X"
            rowText scr 3 `shouldBe` "outside"
